...
pub fn evaluate_sprites<'a, T: Cycle + Sprites + Memory>(
    ...
    move || {
        ...
        for i in 0..SECONDARY_OAM_SPRITE_COUNT {
            ...
            yield;
            let tile_low: u8 = ppu.borrow().get(low_tile_addr);
            yield;
            ...
            yield;
            let tile_high: u8 = ppu.borrow().get(high_tile_addr);
            yield;
            cycle_count += 8;
            result.push(Sprite {
                x_coordinate,
...
fn evaluate_background<'a, T: Sprites>(
    _: &'a RefCell<T>,
...
fn cycle<'a, T: Sprites>(_: &'a RefCell<T>) -> impl Generator<Yield = (), Return = ()> + 'a {
    move || {
        yield;
    }
}
